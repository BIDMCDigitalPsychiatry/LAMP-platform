...
function SearchPage(){const{siteConfig:{baseUrl}}=(0,useDocusaurusContext/* default */.Z)();const{searchValue,updateSearchPath}=hooks_useSearchQuery();const[searchQuery,setSearchQuery]=(0,react.useState)(searchValue);const[searchSource,setSearchSource]=(0,react.useState)();const[searchResults,setSearchResults]=(0,react.useState)();const pageTitle=(0,react.useMemo)(()=>searchQuery?simpleTemplate(proxiedGenerated/* translations.search_results_for */.Iz.search_results_for,{keyword:searchQuery}):proxiedGenerated/* translations.search_the_documentation */.Iz.search_the_documentation,[searchQuery]);(0,react.useEffect)(()=>{updateSearchPath(searchQuery);if(searchSource){if(searchQuery){searchSource(searchQuery,results=>{setSearchResults(results);});}else{setSearchResults(undefined);}}// `updateSearchPath` should not be in the deps,
