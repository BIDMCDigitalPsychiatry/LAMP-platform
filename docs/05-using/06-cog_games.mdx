...
### Memory Game (TicTacToe)
TicTacToe is a simple visual association memory task involving visual, spatial, and temporal associative memory. Participants are first shown a grid of gray boxes. A short sequence of photos from predetermined categories of images replaces boxes within this grid, before disappearing again. Participants are asked to identify both the order in which the photos appeared on the grid, as well as which gray boxes they replaced. This process is repeated multiple times, allowing the participant to learn the temporal sequence and spatial organization of the photos. Once the learning trials have finished, a short delay occurs with a timer on the smartphone, then participants are asked again once more to identify the sequence and spatial organization of the photos, however without the sequence being shown right before as in the learning trials. The design allows for multiple variations, and the task with 3 learning phases can be completed in about 90 seconds, absent from the delay. The number of foils, animation interval, animation persistence, number of encoding trials, and recall delay period can be customized in the mindLAMP app.
...
**Domains assessed: visual, spatial, and temporal memory**
...
#### Scoring
Audio file content is stored as a binary string which can be used for future analysis.

#### Sample Instructions
*Press the microphone to record.* 
*Click upload or clear to record again.*

## In Development

### Digit Span
This task is the same as the Digit Span Forwards/Backwards completed during many standard neuropsychology assessments (Weschler, 2008). A sequence of numbers (0-10) is read aloud by an automated voice and shown one by one on the screen, and participants are asked to repeat the sequence by tapping the numbers on a phone screen, either in the forwards order or the backwards order. This is an adaptive game, meaning that the number of digits in the sequence will change depending on whether the participant got the previous sequence correct.

<details>
    <summary> References </summary>
    
    1. Weschler, D. (2008). Weschler Adult Intelligence Scale—Fourth Edition: Administration and scoring manual. Pearson.

</details>

### Associative Memory Game
The Associative Memory Game, derived from visual association memory tests (Lindeboom et al., 2002), asks participants to recall and recognize the combination between two unrelated items in the same image (e.g. a fox in an ice cream cone). The participant is briefly shown the image and then asked to say out loud and record what two objects were in the image. This process repeats for 6 images. A delay period, customizable within the mindLAMP platform, then takes place during which participants are asked orientation questions (e.g. today’s date, day of the week, season, etc.). Following the delay period, participants are asked to record themselves saying aloud any of the image pairs they can recall. Then, each image is shown with only one object; participants are then asked to say and record what the second object in the image was (e.g. ice cream cone is shown, and participant must say that “fox” was the missing item in the picture). Finally, 4 images slightly differing from each other are presented on the screen, and the participant is asked to tap on the picture that they were originally presented. This last round is repeated for each of the 6 images originally shown on the screen during the learning trial.

<details>
    <summary> References </summary>
    
    1. Lindeboom, J., Schmand, B., Tulner, L., Walstra, G., & Jonker, C. (2002). Visual association test to detect early dementia of the Alzheimer type. Journal of Neurology, Neurosurgery, and Psychiatry, 73(2), 126–133. https://doi.org/10.1136/jnnp.73.2.126
