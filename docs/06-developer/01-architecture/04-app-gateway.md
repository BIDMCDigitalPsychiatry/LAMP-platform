---
title: "App Gateway"
description: "Push notification architecture for iOS and Android."
---
...
External collaborators are **NOT PERMITTED** to use `mailto:`, `sms:`, or `slack:` destinations. [More information on **sending messages through the App Gateway from the command line** can be found below.](#api-usage) 
...
8. the collaborator organization fills in the API Key in their LAMP Platform configuration (see Deployment documentation for more info)
...
The [utility functions provided by Cortex should be preferred](/developer/cortex) over making raw API calls to the App Gateway. However, an example of the raw API call using `cURL` is provided below.
